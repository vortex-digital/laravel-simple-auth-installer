...
namespace LaravelSimple\Installer\Console;
...
class NewCommand extends Command
{
    ...
    protected function configure()
    {
        $this
            ...
            ->addOption('auth', null, InputOption::VALUE_NONE, 'Installs the Laravel login / registration authentication old scaffolding')
            ->addOption('vue-auth', null, InputOption::VALUE_NONE, 'Installs the Laravel login / registration authentication old scaffolding')
            ->addOption('react-auth', null, InputOption::VALUE_NONE, 'Installs the Laravel login / registration authentication old scaffolding')
...
|______\__,_|_|  \__,_| \_/ \___|_|   Simple</>' . PHP_EOL . PHP_EOL);
        ...
        if (($process = $this->runCommands($commands, $input, $output))->isSuccessful()) {
            ...
            if ($input->getOption('auth')) {
                $this->installAuth($directory, 'bootstrap', $input, $output);
            }

            if ($input->getOption('vue-auth')) {
                $this->installAuth($directory, 'vue', $input, $output);
            }

            if ($input->getOption('react-auth')) {
                $this->installAuth($directory, 'react', $input, $output);
            }
    ...
    /**
     * Install Laravel login / register & bootstrap 4 into the application.
     *
     ...
     * @return void
     */
    protected function installAuth(string $directory, string $framework, InputInterface $input, OutputInterface $output)
    {
        chdir($directory);

        $commands = array_filter([
            $this->findComposer() . ' require laravel/ui',
            trim(sprintf(PHP_BINARY . ' artisan ui '. $framework .' --auth')),
